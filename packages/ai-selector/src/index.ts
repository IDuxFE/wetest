...
export default class DomSelector {
    ...
    constructor (options: Options) {
        ...
        this.selectorInfo = options?.selectorInfo ? options.selectorInfo : {
            levelSelectorMap: {},
            firstSelector: []
        }
    }
    ...
    private recursionMathEl (levelSelectorMap: LevelSelectorMap, level: number, selector: string, options: RecursionOptions, targetDirectSelector: string) {
    ...
    private afterAction (el: Element) {
        ...
        this.recursionMathEl(this.selectorInfo.levelSelectorMap, 0, '', {
    ...
    private processElementClass (el: Element) {
        let classArr = el.className.trim().split(/\s+/).filter(cla => !this.selectorCfg.excludeClass.includes(cla) && !this.selectorCfg.excludeClassModify.test(cla))
