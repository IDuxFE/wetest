...
function getElementAccessibleNameInternal(element: Element, options: AccessibleNameOptions): string {
  ...
  if (options.embeddedInLabel !== 'none' || options.embeddedInLabelledBy !== 'none') {
    const isOwnLabel = [...Array.from((element as (HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement)).labels || [])].includes(element as any);
    ...
    if (!isOwnLabel && !isOwnLabelledBy) {
      ...
      if (['combobox', 'listbox'].includes(role)) {
        ...
        if (element.tagName === 'SELECT') {
          selectedOptions = [...Array.from((element as HTMLSelectElement).selectedOptions)];
  ...
  if (!['presentation', 'none'].includes(role)) {
    ...
    if (element.tagName === 'INPUT' && (element as HTMLInputElement).type === 'image') {
      ...
      if (labels.length) {
        return [...Array.from(labels)].map(label => getElementAccessibleNameInternal(label, {
          ...options,
